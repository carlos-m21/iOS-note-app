    ...
    func updateUIViewController(_ uiViewController: UIViewController, context: UIViewControllerRepresentableContext<AlertControlView>) {
        ...
        if self.showAlert {
            ...
            alert.addTextField { textField in
                ...
                textField.text = form.title            // setting initial value
            ...
            alert.addAction(UIAlertAction(title: NSLocalizedString("Submit", comment: ""), style: .default) { _ in
                ...
                alert.dismiss(animated: true) {
                    if form.id != nil {
                        store.update(folderId: form.id!, title: form.title, count: form.count, index: form.index)
                    } else {
                        store.create(title: form.title, index: form.index)
                    }
                    self.showAlert = false
                }
            })
    ...
    class Coordinator: NSObject, UITextFieldDelegate {
        ...
        func textField(_ textField: UITextField, shouldChangeCharactersIn range: NSRange, replacementString string: String) -> Bool {
            if let text = textField.text as NSString? {
                self.control.form.title = text.replacingCharacters(in: range, with: string)
            } else {
                self.control.form.title = ""
            }
            return true
        }
    }
